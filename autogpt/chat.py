...
def chat_with_ai(
    ...
    if api_manager.get_total_budget() > 0.0:
        ...
        if remaining_budget < 0:
            remaining_budget = 0
        system_message = f"Your remaining API budget is ${remaining_budget:.3f}" + (
        ...
        current_context.append(Message("system", system_message))

    # Append user input, the length of this is accounted for above
    current_context.extend([Message("user", user_input)])

    plugin_count = len(cfg.plugins)
    for i, plugin in enumerate(cfg.plugins):
        if not plugin.can_handle_on_planning():
            continue
        plugin_response = plugin.on_planning(agent.prompt_generator, current_context)
        ...
        if current_tokens_used + tokens_to_add > send_token_limit:
            ...
            break
        current_context.append(Message("system", plugin_response))
    ...
    full_message_history.append(Message("user", user_input))
    full_message_history.append(Message("assistant", assistant_reply))
